...
gulp.task('make-all', (app) => {
    ...
    for (let value of appArray) {
        ...
        if (fileExists(existsFile)) console.error(chalk.red(`Error: ${makeStr} already exist! Pls,try other name`))
        else {
            if (!fileExists(blueprint)) console.error(chalk.red(`Error: ${makeStr} blueprint not found`))
            else {
                let replaceValue = ''
                ...
                gulp.src([blueprint])
                    ...
                    .pipe(replace('RouteController', app.charAt(0).toUpperCase() + app.slice(1) + 'Controller'))
                    .pipe(gulp.dest(`app/${app}/`))
                console.log(chalk.green(`Generate ${makeStr} Success`))

                if (value === 'route') {
                    gulp.src('index.js')
                        .pipe(replace('// routeImport', `app.use('/', require('./app/${app}/${app}-route'))\n// routeImport`))
                        .pipe(gulp.dest(''))
                }
